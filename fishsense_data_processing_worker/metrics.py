...
def __system_monitor_loop():
    ...
    wd_info = get_info(
        'service', 'E4E FishSense Data Processing Worker Info',
        namespace='e4efs',
        subsystem='woker')
    wd_info.info({
        'program_version': version('fishsense_data_processing_worker')
    })

    thread_monitor = get_gauge(
        'e4efs_worker_thread_alive', 'Thread Alive Status', ['thread'])
